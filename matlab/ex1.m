...
classdef ex1
    ...
    methods
        ...
        function varargout = getInitialState(~, nominal_trajectory_x)
            ...
            x0_experiment_1 = [0, 0, 0, 3, atan(1e-10 * 3)]';
        ...
        function varargout = getOpenLoopControlSignal(~,sampling_time, simulation_time)
            ...
            uOpenLoop_experiment_1 = [time_vector, ...
                ones(dim,1).*3, ...
                ones(dim,1).*atan(1e-10 * 3)];
            
            %%2nd experiment
            uOpenLoop_experiment_2 = [time_vector, ...
                ones(dim,1).*60, ...
                ones(dim,1).*atan(1e-10 * 3)];
            
            %%3rd experiment
            uOpenLoop_experiment_3 = [time_vector, ...
                ones(dim,1).*.5, ...
                ones(dim,1).*pi/6];
            
            %%4th experiment
            uOpenLoop_experiment_4 = [time_vector, ...
                ones(dim,1).*1, ...
                ones(dim,1).*pi/6];
            
            %%5th experiment
            uOpenLoop_experiment_5 = [time_vector, ...
                ones(dim,1).*3, ...
                ones(dim,1).*pi/6];

            %% 6th experiment
            uOpenLoop_experiment_6 = [time_vector, ...
                ones(dim,1).*3, ...
                time_vector * -pi / (2*time_vector(end))];

            %% 7th experiment
            uOpenLoop_experiment_7 = [time_vector, ...
                ones(dim,1).*3, ...
                time_vector * 2*pi / (time_vector(end))];

            %% 8th experiment
            steering_angle = zeros(size(time_vector));
            steering_angle(1:floor(size(time_vector)/2)) = pi/4;
            steering_angle(floor(size(time_vector)/2):end) = -pi/4;
            uOpenLoop_experiment_8 = [time_vector, ...
                ones(dim,1).*3, ...
                steering_angle];

            %% 9th experiment
            speed = zeros(size(time_vector));
            speed(1:floor(size(time_vector)/2)) = 3;
            speed(floor(size(time_vector)/2):end) = -3;
            uOpenLoop_experiment_9 = [time_vector, ...
                speed, ...
                ones(dim,1).* pi/6];

            %% 10th experiment
            uOpenLoop_experiment_10 = [time_vector, ...
                speed + 6, ...
                ones(dim,1).* pi/6];

            %% 11th experiment
            uOpenLoop_experiment_11 = [time_vector, ...
                speed, ...
                steering_angle];
            
            %Include different values for the different experiments in a
            %cell.
            input_control_actions_open_loop = {...
                uOpenLoop_experiment_1, ...
                uOpenLoop_experiment_2, ...
                uOpenLoop_experiment_3, ...
                uOpenLoop_experiment_4, ...
                uOpenLoop_experiment_5, ...
                uOpenLoop_experiment_6, ...
                uOpenLoop_experiment_7, ...
                uOpenLoop_experiment_8, ...
                uOpenLoop_experiment_9, ...
                uOpenLoop_experiment_10, ...
                uOpenLoop_experiment_11, ...
                };
